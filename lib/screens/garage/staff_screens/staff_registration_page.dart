...
class StaffRegistrationPage extends StatefulWidget {
  @override
  _StaffRegistrationPageState createState() =>
...
class _StaffRegistrationPageState extends State<StaffRegistrationPage> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // Find the selected designation ID
      final designation = designations.firstWhere(
            (d) => d.designation == selectedDesignation,
        orElse: () => Designation(designationId: -1, designation: ''),
      );

      if (designation.designationId == -1) {
        CommonLoaders.errorSnackBar(title: 'Error',message:'Please select a valid designation', duration: 4);
        return;
      }

      final staffModel = StaffRegistrationModel(
        staffName: staffName!,
        staffEmail: staffEmail!,
        staffPhone: staffPhone!,
        designationId: designation.designationId,
        userid: loggedUserid!, // Ensure this is initialized
      );

      try {
        final response = await _staffServices.registerStaff(staffModel);
